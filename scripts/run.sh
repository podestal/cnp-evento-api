...
# daphne -b 0.0.0.0 -p 8000 evento.asgi:application
# gunicorn evento.wsgi:application --bind 0.0.0.0:8000 --timeout=5 --threads=10

if [ "$ENVIRONMENT" = "development" ]; then
    echo "Starting server with regular server for development (supports WebSockets)..."
    exec python manage.py runserver 0.0.0.0:8000
else
    echo "Starting server with Gunicorn for production (supports WebSockets)..."
    exec gunicorn evento.wsgi:application --bind 0.0.0.0:8000 --timeout=5 --threads=10
fi
